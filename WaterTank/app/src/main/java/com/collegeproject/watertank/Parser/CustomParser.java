...
import com.collegeproject.watertank.LoginActivity;
...
//
    ...
    public static ArrayList<NotificationModel> NotificationParse(String response, Context activity) throws JSONException {
        ArrayList<NotificationModel> models = new ArrayList<>();
        JSONArray array = new JSONArray(response);
        for (int i = 0; i < array.length(); i++) {

            NotificationModel notificationModel=new NotificationModel();
            notificationModel.setId(array.getJSONObject(i).getInt("ID"));
            notificationModel.setUser_id(array.getJSONObject(i).getString("user_id"));
            notificationModel.setPH(array.getJSONObject(i).getString("PH"));
            notificationModel.setWater_level(array.getJSONObject(i).getString("water_level"));
            Float phF = Float.parseFloat(notificationModel.getPH());
            int wLI = Integer.parseInt(notificationModel.getWater_level());
            Log.e("Salama", "NotificationParse: "+ Contract.WATER_LEVEL);
            if (phF < 5 || phF > 11 || wLI < LoginActivity.getWaterFromPrefs(activity)) {
